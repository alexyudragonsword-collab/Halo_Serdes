...
所有预设、所有引擎路径,把标量与数组哈希存成基线,改完逐位对。本次两轮重构各比了 **469 个值**。

**吞掉异常会把"坏了"伪装成"没有数据"。** `dual_engine` 把统计引擎失败变成 `rec.stat = None`,
渲染出来和"没跑统计引擎"一模一样 —— 而那个页面就是铁律 #3 的交叉校验,于是**校验坏了**
被显示成**校验没话说**。规则:catch 之后必须有一条通向用户的路径带着原因,
`return None` / `pass` 不算。
