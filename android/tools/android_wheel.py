#!/usr/bin/env python3
# Vendored from the `python-android-apk` Claude skill
# (skills/python-android-apk/scripts/), unmodified except where marked
# "Halo_Serdes:" below. Copied rather than referenced because CI has no skill
# installed and the compiled APK has to build there, not only on a laptop.
#
# Halo_Serdes changes to this file:
#   - the wheel assembler no longer discards every `.c`; see the comment at
#     the `path.suffix == ".c"` test. It used to drop `io/ami_c/halo_fir_ami.c`,
#     which this project publishes as package data, and the only symptom was
#     six `RuntimeError: reference ...` failures in tests/test_ami_c.py.
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
        for path in sorted(tree.rglob("*")):
            if path.is_dir() or path.suffix == ".pyc":
                continue
            # Halo_Serdes: drop only the .c *this script generated* --
            # each of those sits beside the .so it was compiled into. A
            # hand-written .c shipped as package data has no such
            # sibling and must survive. Discarding by suffix alone took
            # io/ami_c/halo_fir_ami.c with it, and the wheel still built
            # clean: the loss only surfaced two layers away, as
            # RuntimeError out of tests/test_ami_c.py.
            if path.suffix == ".c" and path.with_suffix(".so").is_file():
                continue
