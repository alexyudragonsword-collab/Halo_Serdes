...
在这之前,套件里每个测试都直接对着门面说话,**没有任何东西启动过 Activity** ——
"图表到底画没画出来""内置信道芯片有没有到屏幕上",只能装 APK 用眼睛看,
而那是一种一旦有人忘了看就悄悄停止的检查。

`UiRenderTest` 用 `createAndroidComposeRule` 起真的 Activity、跑真的 Python。
两类断言强度不同,别混为一谈:

- **结构**:节点存在,能用可见文本匹配的就用文本 —— 这样**文案也在测试里**。
- **像素**:Canvas 不暴露任何文本,画白的图表和"这条链路没东西可显示"在语义树上一模一样。
  `assertDrew` 读位图、要求有真实的颜色变化。**故意不做 golden image 比对**:
  那种东西在模拟器字体和 GPU 一变就红,而随之而来的"再基线一次"的习惯会让它
  彻底不再是一个检查。

**截图是给人看的旁证,不是 oracle** —— 断言才是让构建失败的东西。它们经 `TestStorage`
交出:`connectedAndroidTest` 跑完会**卸载两个 APK**,写在 app 自己目录里的文件会随之消失,
所以必须在运行期间就交出去。

**一个必须绕开的坑**:Compose 的自动同步等的是**时钟空闲**,而不定态进度条是无限动画 ——
只要它在屏幕上,`performClick` 这类调用会**阻塞到 job 超时**而不是给一条能读的失败。
这个 app 每次调 Python 都会亮一下 spinner,所以这是主路径。所有不定进度条打
`TestTags.BUSY`,交互前先 `waitUntil { 没有 BUSY 节点 }`。
